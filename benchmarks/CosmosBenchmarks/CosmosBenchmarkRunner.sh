...
Messages=(1000 10000 50000)
Subscribers=(1 2 4 8 16)
iterationsPerBenchmark=1

outputFile=test_Benchmarks.txt
...
for msg in "${Messages[@]}";
  ...
  for subs in "${Subscribers[@]}";
    ...
    do
      ...
      dotnet run --project CosmosBenchmarks.csproj "$msg" "$subs" "output" >> "$logFile" 2>> "$outputFile"
    done
  done  
done
